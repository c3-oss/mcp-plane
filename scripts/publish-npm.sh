...
update_version "$NPM_ROOT/mcp-plane/package.json"
for p in $PLATFORMS; do
    update_version "$NPM_ROOT/mcp-plane-$p/package.json"
done

# 2. Copy the matching binary into each platform package.
#    Goreleaser dist layout: dist/mcp-plane_<os>_<arch>(_<microarch>)/mcp-plane
for p in $PLATFORMS; do
    os=${p%-*}
    arch=${p#*-}
    src=$(find "$DIST" -path "*/mcp-plane_${os}_${arch}*/mcp-plane" -type f | head -n 1)
    if [ -z "$src" ]; then
        echo "missing dist artifact for $p" >&2
        exit 1
    fi
    dst=$NPM_ROOT/mcp-plane-$p/bin/mcp-plane
    mkdir -p "$(dirname "$dst")"
    cp "$src" "$dst"
    chmod 0755 "$dst"
done
...
for pkg in "$NPM_ROOT/mcp-plane/package.json" \
           ...
           "$NPM_ROOT/mcp-plane-linux-amd64/package.json" \
           "$NPM_ROOT/mcp-plane-linux-arm64/package.json"; do
